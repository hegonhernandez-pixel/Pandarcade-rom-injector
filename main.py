...
class PandarcadeInjectorGUI:
    def __init__(self, root):
        ...
        self.sistema_seleccionado = tk.StringVar(value="fba")
    ...
    def crear_componentes(self):
        ...
        # --- PANEL DE EMULADORES ---
        frame_ems = tk.LabelFrame(self.root, text=" 🎮 Filtro de Sistema / Emulador Integrado ", font=("Arial", 10, "bold"), padx=15, pady=10)
        frame_ems.pack(fill="x", padx=20, pady=5)
        
        sistemas = [("FinalBurn Alpha (fba)", "fba"), ("MAME Arcade (mame)", "mame"), ("Sega MegaDrive (md)", "md"), ("Nintendo NES (fc)", "fc")]
        for i, (texto, valor) in enumerate(sistemas):
            tk.Radiobutton(frame_ems, text=texto, variable=self.sistema_seleccionado, value=valor, font=("Arial", 9)).grid(row=0, column=i, padx=8, pady=5)
        ...
        self.txt_logs = tk.Text(frame_logs, height=8, width=75, font=("Consolas", 9), bg="#2f3542", fg="#7bed9f")
        ...
        self.log("Sistema Pandarcade listo. Esperando asignación de rutas...")
    ...
    def ejecutar_purga_en_crudo(self):
        # Este método enlazará directamente los scripts de automatización limpia que diseñamos
        orig = self.ruta_origen.get()
        dest = self.ruta_destino.get()
        if not orig or not dest:
            messagebox.showerror("Error de Configuración", "Por favor, asigna ambas rutas antes de iniciar la purga.")
            return
        self.log("🚀 Iniciando Purga Quirúrgica... Eliminando archivos duplicados y basura multimedia.")
        # Aquí se ejecutará la lógica en crudo libre de bloqueos de red

if __name__ == "__main__":
    root = tk.Tk()
    app = PandarcadeInjectorGUI(root)
    
    # Botón de ejecución maestro abajo en la ventana
    btn_ejecutar = tk.Button(root, text="🔥 INICIAR OPTIMIZACIÓN Y MIGRACIÓN EN CRUDO", 
                             font=("Arial", 11, "bold"), bg="#2ed573", fg="white", 
                             padx=20, pady=8, command=app.ejecutar_purga_en_crudo)
